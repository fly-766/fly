...
abstract contract HopSwap {
    ...
    error UnsafeHop();
    constructor(address router, IERC20 quoteToken, IERC20 wrappedOkb, IERC20 usd0Token) {
        if(address(usd0Token)==address(0)||address(usd0Token).code.length==0)revert UnsafeHop();
        if(address(quoteToken)==address(usd0Token)){
            if(router!=address(0)||address(wrappedOkb)!=address(0))revert UnsafeHop();
            hopRouter=address(0);quote=usd0Token;wokb=IERC20(address(0));return;
        }
        if(router==address(0)||router.code.length==0||address(quoteToken).code.length==0||address(wrappedOkb).code.length==0)revert UnsafeHop();
        ...
        hopRouter=router;quote=quoteToken;wokb=wrappedOkb;
    }
    function _hop(IERC20 src, IERC20 dst, uint256 amount, uint256 minOut, bytes memory data) internal returns(uint256 received) {
        ...
        if(hopRouter==address(0)||data.length==0||amount==0||minOut==0)revert UnsafeHop();
        ...
        src.forceApprove(hopRouter,amount);
        (bool ok,)=hopRouter.call(data);
        src.forceApprove(hopRouter,0);
        ...
        if(address(wokb)!=address(0)&&wokb.balanceOf(address(this))!=0)revert UnsafeHop();
    }
}
