...
import {ICoreRead03,ICoreWriter03,ICoreDeposit03,ICctpMessenger,ISettlement03,IPrincipalReceiver,CoreEncoding} from "./ProtocolTypes.sol";
import {RunCommitRegistry} from "./RunCommitRegistry.sol";
import {CoreExit} from "./CoreExit.sol";
contract TradingAccount is ReentrancyGuard,IPrincipalReceiver {
    ...
    struct Config {
        ...
        uint32 returnDomain;uint64 maxOrderE6;uint64 lossStopE6;uint32 orderCooldown;uint16 maxOrdersPerDay;uint16 slippageBps;uint16 returnFeeBps;uint64 emergencyDelay;
    }
    ...
    uint64 public immutable maxOrderE6;uint64 public immutable lossStopE6;uint32 public immutable orderCooldown;uint16 public immutable maxOrdersPerDay;uint16 public immutable slippageBps;uint64 public immutable emergencyDelay;
    ...
    uint64 public lastCommit;uint64 public lastOrderAt;uint256 public day;uint256 public ordersToday;uint256 public nativeBasisE6;bool public setupComplete;bool public accountingQuarantined;bool public paused;bool public recoveryOnly;uint64 public pausedAt;
    ...
    constructor(Config memory c) {
        if(address(c.usdc).code.length==0||address(c.reader).code.length==0||address(c.writer).code.length==0||address(c.depositWallet).code.length==0||
           ...
           c.maxOrderE6<10e6||c.lossStopE6==0||c.slippageBps>100||c.orderCooldown<20||c.maxOrdersPerDay==0||c.maxOrdersPerDay>100||c.emergencyDelay<1 days)revert Restricted();
        ...
        maxOrderE6=c.maxOrderE6;lossStopE6=c.lossStopE6;slippageBps=c.slippageBps;orderCooldown=c.orderCooldown;maxOrdersPerDay=c.maxOrdersPerDay;emergencyDelay=c.emergencyDelay;
    ...
    function moveToPerp() external nonReentrant {
        ...
        writer.sendRawAction(abi.encodePacked(uint8(1),uint24(7),abi.encode(uint64(amount),true)));
    }
    ...
    function _pause(bool recovery) internal {paused=true;if(pausedAt==0)pausedAt=uint64(block.timestamp);if(recovery)recoveryOnly=true;emit Paused(recoveryOnly,pausedAt);}
    ...
    function _order(bool buy,uint64 limit,uint64 size,ICoreRead03.State memory s) internal {
        ...
        writer.sendRawAction(abi.encodePacked(uint8(1),uint24(1),abi.encode(uint32(0),buy,limit,size,!buy,uint8(3),cloid)));
    ...
    function cancelAndPause() external nonReentrant {
        ...
        if(pending==PendingKind.ORDER)writer.sendRawAction(abi.encodePacked(uint8(1),uint24(11),abi.encode(uint32(0),cloid)));
    }
    ...
    function requestExit(uint256 amount,bool recovery) external nonReentrant {
        ICoreRead03.State memory s=_state();if(pending!=PendingKind.NONE||s.quantity!=0||!s.exists||amount<2e6||amount>type(uint64).max/100)revert Restricted();
        if(recovery){if(msg.sender!=guardian||!paused||block.timestamp<pausedAt+emergencyDelay||s.equityE6<=0||amount>uint256(uint64(s.equityE6))||unallocatedSpotE6!=0||amount>costBasisE6-nativeBasisE6)revert Restricted();recoveryOnly=true;}
    ...
    function quarantineUnknown(bytes32 evidence) external {
        if(msg.sender!=guardian||!paused||block.timestamp<pausedAt+emergencyDelay||evidence==0||
    ...
    function recoverExit(bool isRecovery,bool allowReset) external {
        if(msg.sender!=guardian||!recoveryOnly||block.timestamp<pausedAt+emergencyDelay)revert Restricted();
        (isRecovery?recoveryExit:exit).emergencyRecover(allowReset);
    }
    function recoverSpot(uint256 amount) external nonReentrant {
        if(msg.sender!=guardian||!paused||block.timestamp<pausedAt+emergencyDelay||pending!=PendingKind.NONE||amount<2e6||amount>costBasisE6-nativeBasisE6||(!accountingQuarantined&&amount>unallocatedSpotE6)||amount>type(uint64).max/100)revert Restricted();
        (uint64 balance,uint64 hold)=reader.spot(address(this),0);if(uint256(balance)-hold<amount*100)revert Restricted();
        recoveryOnly=true;ICoreRead03.State memory state_=_state();_start(PendingKind.EXIT,amount,state_);exitKind=2;
    ...
    function recoverNative() external nonReentrant {
        if(msg.sender!=guardian||!paused||block.timestamp<pausedAt+emergencyDelay||pending!=PendingKind.NONE)revert Restricted();
        recoveryOnly=true;
