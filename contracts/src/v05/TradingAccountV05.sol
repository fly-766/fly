...
import {CoreExitV04 as CoreExit} from "../v04/CoreExitV04.sol";
import {CoreReserves} from "../v04/CoreReserves.sol";
import {ICoreRead05,PositionMathV05} from "./PositionMathV05.sol";
/// @notice Principal exit has no project-imposed timelock; venue settlement must still finish.
contract TradingAccountV05 is CoreReserves,IPrincipalReceiver {
    ...
    struct Config {
        ...
        uint32 returnDomain;uint64 maxOrderE6;uint64 lossStopE6;uint32 orderCooldown;uint16 maxOrdersPerDay;uint16 slippageBps;uint16 returnFeeBps;uint8 leverageCap;uint16 reserveBps;
    }
    ...
    constructor(Config memory c) {
        if(address(c.usdc).code.length==0||address(c.reader).code.length==0||address(c.writer).code.length==0||address(c.depositWallet).code.length==0||
           ...
           c.leverageCap==0||c.leverageCap>20||c.reserveBps<1000||c.reserveBps>5000||c.maxOrderE6<10e6||c.lossStopE6==0||c.slippageBps>100||c.orderCooldown<20||c.maxOrdersPerDay==0||c.maxOrdersPerDay>100)revert Restricted();
        ...
        leverageCap=c.leverageCap;reserveBps=c.reserveBps;maxOrderE6=c.maxOrderE6;lossStopE6=c.lossStopE6;slippageBps=c.slippageBps;orderCooldown=c.orderCooldown;maxOrdersPerDay=c.maxOrdersPerDay;
    ...
    function previewOrder(bool buy) public view returns(uint64 priceE8,uint64 sizeE8) {
        ...
        if(s.quantity==0&&(!buy&&!shortEnabled||unallocatedSpotE6!=0||_loss(s)))revert Restricted();
        return ICoreRead05(address(reader)).quoteOrder(address(this),buy,maxOrderE6,leverageCap,reserveBps,slippageBps);
    }
    function execute(uint64 commit) external nonReentrant {
        ...
        if(PositionMathV05.signedSize(s)!=settledPositionE8)revert Unresolved();
        bool opening=s.quantity==0;
        ...
        if(opening&&(paused||block.timestamp<lastOrderAt+orderCooldown))revert Restricted();
        uint256 today=block.timestamp/1 days;if(day!=today){day=today;ordersToday=0;}
        if(opening&&ordersToday>=maxOrdersPerDay)revert Restricted();
    ...
    function _order(bool buy,uint64 limit,uint64 size,ICoreRead03.State memory s) internal {
        ...
        pendingReduceOnly=s.quantity!=0;entryBeforeE6=s.entryNotionalE6;limitPriceE8=limit;
    ...
    function settleOrder(OrderReceipt calldata r,uint64 coreBlock,uint64 deadline,bytes32 evidence,bytes calldata sig) external {
        ...
        if(r.filledE8==0){if(r.averagePriceE8!=0||r.closedPnlE6!=0||r.feeE6!=0)revert Restricted();}
            ...
            if(!pendingReduceOnly){if(r.closedPnlE6!=0)revert Restricted();}
            else{
