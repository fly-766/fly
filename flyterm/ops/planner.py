...
def hyper_step(snap,cfg):
    ...
    if cfg.get("accountVersion",3)>=5 and s["core"]["quantity"]*10**(8-s["core"]["sizeDecimals"])!=s["settledPositionE8"] and not s["accountingQuarantined"]:
        return action(chain,at,"quarantinePositionDrift()",kind="position_drift_quarantine")
    ...
    if s["core"]["equityE6"]+s["nativePrincipalE6"]+s.get("unallocatedSpotE6",0)+s.get("lossStopE6",cfg.get("lossStopE6",200_000_000))<=s["costBasisE6"] and s["core"]["quantity"]!=0:return action(chain,at,"emergencyClose()")
...
def xlayer_step(snap,cfg,quotes=None):
    ...
    if cfg.get("allowNewCapital",True) and not get(conv,"paused()",["bool"]):
        vault=get(p["manager"],"vaultOf(address)",["address"],["address"],[token])
        if int(vault,16) and cfg.get("localFixtureOnly") and not cfg.get("creatorIncome",{}).get("vault"):
        ...
        if address(quote)==address(p["usd0"]):
            ...
            if not cfg.get("localFixtureOnly"):
                from .hop import QuoteUnavailable
                if quotes is None:return {"wait":"okx_quote_service_required"}
                try:q=quotes.quote(quote,p["usd0"],amount,conv,cfg)
                except QuoteUnavailable:return {"wait":"okx_quote_unavailable"}
                plan=action(chain,conv,"convert(uint256,uint256,bytes)",["uint256","uint256","bytes"],[amount,q["minimum"],bytes.fromhex(q["data"][2:])],"tax_convert")
                plan["intent"]["hopQuote"]=q
                return plan
    ...
    if cfg.get("allowNewCapital",True) and not get(treasury,"paused()",["bool"]):
    ...
    if buffered:
        ...
        if address(quote)!=address(p["usd0"]):
            ...
            if not cfg.get("localFixtureOnly"):
                from .hop import QuoteUnavailable
                if quotes is None:return {"wait":"okx_quote_service_required"}
                try:q=quotes.quote(p["usd0"],quote,buffered,buy,cfg)
                except QuoteUnavailable:return {"wait":"okx_quote_unavailable"}
                plan=action(chain,buy,"hopProfit(uint256,bytes)",["uint256","bytes"],[q["minimum"],bytes.fromhex(q["data"][2:])],"profit_hop")
                plan["intent"]["hopQuote"]=q
                return plan
...
def neural_actionable(side,state,cfg,timestamp):
    """Close opposite exposure first. A new entry requires a new commit and cooldown."""
    q=state["core"]["quantity"]
    if side not in (1,2):return False
    if q>0:return side==2
    if q<0:return cfg.get("accountVersion",3)>=5 and side==1
    if state.get("paused") or state.get("recoveryOnly"):return False
    if side==2 and (cfg.get("accountVersion",3)<5 or not state.get("shortEnabled",True)):return False
    limit=state.get("maxOrdersPerDay",cfg.get("maxOrdersPerDay",24))
    cooldown=state.get("orderCooldown",cfg.get("orderCooldown",20))
    return (state["day"]!=timestamp//86400 or state["ordersToday"]<limit) and timestamp>=state["lastOrderAt"]+cooldown
